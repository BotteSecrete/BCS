...
{	
	...
	if (isGene){isGene = false;} 
	else if (isNotFCond){isNotFCond = false;}
	else if (isNotFStruct){isNotFStruct = false;}
	else if (isAtt){isAtt = false;}
	else
	{
...
{
//	Alert.show("myFeatureLayer_editsCompleteHandler (HandleBatiLayer)","Debugg");
