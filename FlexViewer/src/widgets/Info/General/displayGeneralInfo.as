...
//	genUBInput.addEventListener(FlexEvent.VALUE_COMMIT, saveInfoEditedHandler);
	...
	{
		Alert.show("avant onCbGeneralChange","Debugg");
		onCbGeneralChange(ev, event, k);
		if(!willTrigger(FeatureLayerEvent.EDITS_STARTING))
		{
			Alert.show("dans le if !willTrigger","Debugg");
	...
	{
		Alert.show("debut myFeatureLayer_editsStarted","Debugg");
		...
		if(!clickToAdd && !clickToMove  && !coordToMove) //Cas où l'on update un élément sans le bouger
		{
			if(ev.featureEditResults.updateResults.length > 0)
			{
				...
				function featLayer_updateEndHandler(evt:LayerEvent):void
				{
					...
					Alert.show("featLayer_updateEndHandler  ind = "+ind,"Debugg");
...
//		
//		attachButLoc1.visible = false;
//		attachButOver1.visible = false;
//		attachButOver2.visible = false;
//		attachButOver3.visible = false;
//		attachButOver4.visible = false;
//	}
