...
private function saveInfoEdited(ev:FlexEvent, event:FeatureLayerEvent, k:Number, service:HTTPService):void
{
	...
	{
		case "locaId":
			saveTI(ev.currentTarget,attributes,"LOCA_ID", event.features[k],0,true);
			break;
		
		case "localXInput":
			saveTI(ev.currentTarget,attributes,"LOCA_LOCX",event.features[k],3);
			break;
		
		case "localYInput":
			saveTI(ev.currentTarget,attributes,"LOCA_LOCY",event.features[k],3);
			break;
		
		case "groundLevelInput":
			saveTI(ev.currentTarget,attributes,"LOCA_GL",event.features[k],2);
			break;
		
		case "localZInput":
			saveTI(ev.currentTarget,attributes,"LOCA_LOCZ",event.features[k],3);
			break;
		
		case "holeDepthInput":
			saveTI(ev.currentTarget,attributes,"LOCA_FDEP",event.features[k],2);
			break;
		
		case "CoordSysInput":
			saveTI(ev.currentTarget,attributes,"LOCA_LREF",event.features[k])
			break;
		
		case "genRemInput":
			saveTI(ev.currentTarget,attributes,"LOCA_REM",event.features[k])
			break;
		
		case "investPhaseInput":
			saveTI(ev.currentTarget,attributes,"LOCA_CLST",event.features[k])
			break;
		
		case "otherSiteLoca":
			if(!(locaLoca.Site.contains(ev.target.text))){
				
				locaLoca.appendChild(<Site>{ev.target.text}</Site>);
				var writeXML:HTTPService = new HTTPService(); 	
				writeXML.url = "customRenderer/addNodeSiteLocation.aspx";
				writeXML.method = "POST";
				
				
				var _sDataFileFullFileSpec:String = "https://sygdev.systra.info/LOCA_LOCA.xml";	//so we can do save as by changing the file spec
				var oRequest:Object = new Object();	//same as get
				oRequest.getorput = "put"
				oRequest.fullfilespec = _sDataFileFullFileSpec;								
				oRequest.content = locaLoca.toXMLString();
				var callResponse:Object = writeXML.send(oRequest);
				writeXML.addEventListener(ResultEvent.RESULT, resultHandlerWrittingXML); 
				callResponse.getorput = "put";
				cursorManager.setBusyCursor();
				function resultHandlerWrittingXML(oEvent:ResultEvent):void
				{	
					service.send();
				}
			}
			saveTI(ev.currentTarget,attributes,"LOCA_LOCA",event.features[k]);
			break;
	}
