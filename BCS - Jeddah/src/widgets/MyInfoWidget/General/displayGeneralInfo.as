...
//	FIList.addEventListener(ListEvent.CHANGE, onCbChangeHandler);
	...
	var LabelReport:mx.controls.Label = new mx.controls.Label();
	LabelReport.text = "Report : ";
	var attachButReport:Button = new Button();
	attachButReport.name = "Report";
	attachButReport.label = "Report ";
	attachButReport.addEventListener(MouseEvent.CLICK,editAttachFeaturesHandler);
	gridItemUR41.addChild(LabelReport);gridItemUR42.addChild(attachButReport);
	gridRowUR4.addChild(gridItemUR41);gridRowUR4.addChild(gridItemUR42);
	
	gridUR.addChild(gridRowUR1);gridUR.addChild(gridRowUR2);gridUR.addChild(gridRowUR4);
...
//			attachFeat.addEventListener(FeatureLayerEvent.EDITS_STARTING, myAttachFeatureLayer_editsStarted);
	...
	function tooltipMaker():void
	{
		if (isInAttach)
		{
			...
			else if (isReport) {setTooltip("Report");}
	...
	function addAttachInfo(event:FeatureLayerEvent, k:Number,idpjs:String):void
	{
		...
		else if (isReport) {attributes = {IDBATI:event.features[k].attributes.OBJECTID.toString(),IDPJ:idpjs,Theme:"Report"};}
