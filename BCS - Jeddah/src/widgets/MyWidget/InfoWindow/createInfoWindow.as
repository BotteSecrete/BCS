...
//	IDItem.required = true;
	...
	deleteBut.name="Delete";
	deleteBut.addEventListener(MouseEvent.CLICK,deleteFeatureHandler);
	
	var editBut:Button = new Button();
	editBut.label="Edit Draw Buildings";
	editBut.name="Edit";
	editBut.addEventListener(MouseEvent.CLICK,editFeatureHandler);
	
	hBoxBtn.addChild(GnrlBtn);
	hBoxBtn.addChild(StructBtn);
	hBoxBtn.addChild(editBut);
	hBoxBtn.addChild(deleteBut);
...
//	form.addChild(hboxYC);
	...
	if(!index.collapsingPanel.collapsed) 
	{
		...
		index.ws0.width = index.collapsingPanel.explicitWidth; 
		form.width = index.ws0.width - 10;
		form.height = index.ws0.height;
	...
	function editFeatureHandler(ev:MouseEvent):void
	{
		if (ev.currentTarget.label == "Edit Draw Buildings")
		{
			ev.currentTarget.label = "End Edition";
			map.removeEventListener(MapMouseEvent.MAP_CLICK,map_mapClickHandler);
			map.addEventListener(MapMouseEvent.MAP_MOUSE_DOWN,drawEditPolygon);
		}else if (ev.currentTarget.label == "End Edition")
		{
			ev.currentTarget.label = "Edit Draw Buildings";
			myEditTool.deactivate();
			map.addEventListener(MapMouseEvent.MAP_CLICK,map_mapClickHandler);
			map.removeEventListener(MapMouseEvent.MAP_MOUSE_DOWN,drawEditPolygon);
			updateDrawPolygon(event.features[k].attributes.OBJECTID);
		}
	}
