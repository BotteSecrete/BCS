package 
{
	...
	[IconFile("ImageRadioButton.png")]
	
	/**
	 * Extends the RadioButton class to add support for showing an image between the 
	 * radiobutton and the label.
	 * 
	...
	public class ImageRadioButton extends RadioButton
	{
		...
		override protected function measure():void {
			...
			if (image.source) {
				...
				if ((iw > 0) && (ih > 0)) {
					...
					var radiobuttion:IFlexDisplayObject = mx_internal::getCurrentIcon();
					var cbw:Number = (radiobuttion ? radiobuttion.width + hgap : 0);
					var cbh:Number = (radiobuttion ? radiobuttion.height : 0);
		...
		/**
		 * Positions the image between the radiobuttion and the label.
		 */
		protected function positionImage(w:Number, h:Number):void {
			if (image.source) {
				...
				// center vertically on the radiobuttion
				if (radiobuttion) {
					iy = Math.max(0, radiobuttion.y + ((cbh - ih) / 2));
				}
				...
				if (labelPlacement == ButtonLabelPlacement.RIGHT) {
					...
					if (radiobuttion) {
						radiobuttion.x += iw + hgap + 2;
					}
				} else {
					// position the image to the right of the radiobuttion
					ix = padLeft + cbw + 2;
				}
